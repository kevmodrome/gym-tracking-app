...
{#if open}
	...
	>
		...
		>
			...
			{#if footer}
				<div class="border-t border-border bg-surface-elevated/50 flex flex-col-reverse sm:flex-row items-stretch sm:items-center justify-end gap-3 flex-shrink-0 {fullScreenMobile ? 'p-3 pb-24 sm:p-6 sm:pb-6' : 'p-6'}">
					{@render footer()}
				</div>
			{/if}
		</div>
